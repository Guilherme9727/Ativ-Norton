...
export class Vitrine {
  ...
  adicionarCesta(produto: Produto): void {
    if (typeof localStorage === 'undefined' || produto.quantidade <= 0) return;

    const json = localStorage.getItem('cesta');
    const itens: ItemCesta[] = json ? JSON.parse(json) : [];
    ...
    for (const item of itens) {
      if (item.produto.codigo === produto.codigo) {
        item.quantidade += 1;
        item.valorTotal = Number((item.quantidade * preco).toFixed(2));
        encontrado = true;
        break;
      }
    }
